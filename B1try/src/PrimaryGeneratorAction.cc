...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* event)
{
  ...
  const G4double sourceRadius = 2500.0 * mm;
  ...
  const G4ThreeVector position(sourceRadius * sinTheta * std::cos(phi),
                               sourceRadius * sinTheta * std::sin(phi),
                               sourceRadius * cosTheta);

  fParticleGun->SetParticlePosition(position);
  fParticleGun->SetParticleMomentumDirection(-position.unit());
